...
class ConfigHA:
    def __init__(self, config_general):
        ...
        # [수정] 안내견-시각장애인 페어 설정: 기존 제어 주체(로봇=안내견)는 유지하고,
        #        동반자(orange circle) 파라미터를 별도 config로 추가
        guide = ConfigBase()
        guide.enable_pair = True
        guide.vip_radius = 0.3
        guide.harness_length_nominal = 0.65
        guide.harness_length_min = 0.45
        guide.harness_length_max = 0.90
        self.guide = guide
