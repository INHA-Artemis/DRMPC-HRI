...
class HAEnv(CrowdSim):
    ...
    def step(self, robot_action, no_pivot=True):
        human_actions = self.get_human_actions()

        # apply action and update all agents
        applied_action = robot_action
        if no_pivot and hasattr(self.config_HA, 'guide') and self.config_HA.guide.enable_pair:
            # [수정] 안내견 제자리 pivot 억제: v가 낮을수록 w를 줄여 회전을 완화
            v_gate = max(self.config_HA.guide.dog_no_pivot_min_v_mps, 1e-6)
            turn_scale = min(abs(robot_action.v) / v_gate, 1.0)
            applied_action = ActionRot(robot_action.v, robot_action.w * turn_scale)
        self.robot.step(applied_action)
        # [수정] 로봇 이동 후 하니스 동역학으로 vip 상태 업데이트
        self.update_vip_with_harness_dynamics()
        if self.robot_valid_history == self.lookback+1:
            self.robot_pose_history_global = np.roll(self.robot_pose_history_global, -1, axis=0)
            self.robot_pose_history_global[-1, :] = np.array([self.robot.px, self.robot.py, self.robot.theta])
            self.robot_past_velocities = np.roll(self.robot_past_velocities, -1, axis=0)
            self.robot_past_velocities[-1, :] = np.array([applied_action.v, applied_action.w])
        else:
            self.robot_pose_history_global[self.robot_valid_history] = np.array([self.robot.px, self.robot.py, self.robot.theta])
            self.robot_past_velocities[self.robot_valid_history-1] = np.array([applied_action.v, applied_action.w])
            self.robot_valid_history = min(self.robot_valid_history + 1, self.lookback+1)
    ...
    def calc_reward(self):
        ...
        for i, human in enumerate(self.humans):
            ...
            dist_to_robot = (dx ** 2 + dy ** 2) ** (1 / 2) - human.radius - self.robot.radius
            if self.vip is not None and hasattr(self.config_HA, 'guide') and self.config_HA.guide.enable_pair:
                # [수정] 충돌/안전거리 계산을 dog 단일체가 아닌 dog+vip 최소거리로 확장
                dx_vip = human.px - self.vip.px
                dy_vip = human.py - self.vip.py
                dist_to_vip = (dx_vip ** 2 + dy_vip ** 2) ** (1 / 2) - human.radius - self.vip.radius
                closest_dist = min(dist_to_robot, dist_to_vip)
            else:
    ...
    def reset(self, path=None):
        ...
        # [수정] 에피소드 시작 시점에도 vip를 로봇 기준으로 초기 정렬
        self.sync_vip_with_robot()
