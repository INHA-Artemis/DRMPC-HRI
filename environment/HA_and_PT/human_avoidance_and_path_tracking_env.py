...
class HAAndPTEnv:
    ...
    def configure_HA(self):
        ...
        # [수정] 상위 통합 env에서 vip 참조를 보관해 렌더/기록에 사용
        self.vip = self.HA_env.vip
    ...
    def create_video_continuous_task(self, vid_name, S_PT):
        ...
        # [수정] 비디오 상태 기록 포맷: [dog_state, human_states, vip_state]
        self.states.append([self.robot.get_full_state(), human_state_list, self.vip.get_full_state()])
        self.harness_tensions = [self.HA_env.harness_tension]
        init_dog_speed = abs(self.robot.v) if hasattr(self.robot, 'v') else 0.0
        init_vip_speed = np.linalg.norm([self.vip.vx, self.vip.vy])
        self.dog_speeds = [init_dog_speed]
        self.vip_speeds = [init_vip_speed]
        self.vip_accels = [0.0]
        init_roll = np.degrees(np.arccos(np.clip(np.linalg.norm([self.HA_env.vip_offset_back, self.HA_env.vip_offset_right]) / max(self.config_HA.guide.rod_length_m, 1e-6), 0.0, 1.0)))
        self.harness_roll_degs = [init_roll]
        ...
        while done:
            ...
            if 'goal' in info['done'] or 'deviation_end_of_path' in info['done']:
                ...
                if self.create_vid:
                    ...
                    human_state_list = [human.get_full_state() for human in self.HA_env.humans]
                    self.states.append([self.robot.get_full_state(), human_state_list, self.vip.get_full_state()])
                    self.harness_tensions.append(self.HA_env.harness_tension)
                    dog_speed = abs(self.robot.v) if hasattr(self.robot, 'v') else 0.0
                    vip_speed = np.linalg.norm([self.vip.vx, self.vip.vy])
                    prev_vip_speed = self.vip_speeds[-1] if len(self.vip_speeds) > 0 else vip_speed
                    vip_acc = (vip_speed - prev_vip_speed) / max(self.time_step, 1e-6)
                    self.dog_speeds.append(dog_speed)
                    self.vip_speeds.append(vip_speed)
                    self.vip_accels.append(vip_acc)
                    roll_deg = np.degrees(np.arccos(np.clip(np.linalg.norm([self.HA_env.vip_offset_back, self.HA_env.vip_offset_right]) / max(self.config_HA.guide.rod_length_m, 1e-6), 0.0, 1.0)))
                    self.harness_roll_degs.append(roll_deg)
    ...
    def set_robot_reset(self, start_angle=None):
        if self.config_HA.sim.scenario == 'circle_crossing':
            if self.robot.kinematics == 'unicycle':
                ...
                # [수정] 하드 리셋에서 dog pose 설정 후 vip도 즉시 동기화
                self.HA_env.sync_vip_with_robot()
            else:
                raise NotImplementedError
        else:
            raise NotImplementedError
    ...
    def step_continuous_task(self, action, update=True, soft_reset=False, model_info=None):
        # [수정] soft_reset 중에는 복구를 위해 no-pivot 제약을 해제
        S_prime_HA, R_HA, done_HA, info_HA = self.HA_env.step(action, no_pivot=not soft_reset)
        ...
        # [수정] VIP(주황 원)도 벽/코리더 위반 시 PT 패널티 부여
        vip_pos = np.array([self.vip.px, self.vip.py])
        vip_hit, vip_dist = self.PT_env.query_off_path(vip_pos, self.config_PT.rewards.params['corridor_hit']['max_deviation'])
        vip_safety, _ = self.PT_env.query_off_path(vip_pos, self.config_PT.rewards.params['safety_corridor_raise']['max_deviation'])
        if vip_hit:
            vip_pen = self.config_PT.rewards.params['corridor_hit']['penalty'] * self.config_PT.rewards.overall_scaling
            R_PT += vip_pen
            done_PT = True
            info_PT['corridor_hit'] += vip_pen
            info_PT['done'].add('corridor_hit')
            info_PT['vip_corridor_hit'] = vip_pen
        else:
            info_PT['vip_corridor_hit'] = 0.0

        if vip_safety:
            vip_safety_pen = self.config_PT.rewards.params['safety_corridor_raise']['penalty'] * self.config_PT.rewards.overall_scaling
            R_PT += vip_safety_pen
            done_PT = True
            info_PT['safety_corridor_raise'] += vip_safety_pen
            info_PT['done'].add('safety_corridor_raise')
            info_PT['vip_safety_corridor_raise'] = vip_safety_pen
        else:
            info_PT['vip_safety_corridor_raise'] = 0.0
        info_PT['vip_dist_to_path'] = vip_dist
        if self.create_vid and self.MPC is not None:
    ...
    def cvmm_safety_check(self, action_execute, num_steps):
        ...
        for i in range(num_steps):
            ...
            distance_to_dog = np.linalg.norm(current_human_positions - np.expand_dims(current_position, axis=0), axis=1)
            if self.vip is not None and hasattr(self.config_HA, 'guide') and self.config_HA.guide.enable_pair:
                # [수정] 안전 파이프라인 예측 시 dog뿐 아니라 vip 위치도 함께 충돌검사
                back = self.HA_env.vip_offset_back if self.HA_env.vip_offset_back is not None else self.config_HA.guide.offset_back_m
                right = self.HA_env.vip_offset_right if self.HA_env.vip_offset_right is not None else self.config_HA.guide.offset_right_m
                cos_th = np.cos(current_pose[2])
                sin_th = np.sin(current_pose[2])
                e_f = np.array([cos_th, sin_th])
                e_r = np.array([sin_th, -cos_th])
                vip_position = current_position - back * e_f + right * e_r
                distance_to_vip = np.linalg.norm(current_human_positions - np.expand_dims(vip_position, axis=0), axis=1)
                distance = np.minimum(distance_to_dog, distance_to_vip)
            else:
                distance = distance_to_dog
    ...
    def render(self):
        ...
        # [수정] vip 궤적 추출
        vip_positions = [np.array([state[2].px, state[2].py]) for state in self.states]
        ...
        # [수정] orange circle 렌더 객체
        vip = plt.Circle(vip_positions[0], self.vip.radius, fill=True, color=vip_color)
        harness_line = mlines.Line2D(
            [robot_positions[0][0], vip_positions[0][0]],
            [robot_positions[0][1], vip_positions[0][1]],
            color=harness_color,
            linewidth=3,
        )
        ax.add_artist(start)
        ...
        ax.add_artist(robot)
        ax.add_artist(vip)
        ax.add_artist(harness_line)
        # [수정] 범례에 dog/vip 분리 표기
        plt.legend([robot, vip, harness_line, start, goal], ['Dog', 'Visually Impaired', 'Harness', 'Start', 'Goal'], fontsize=16, loc='upper right')

        harness_tension_text = None
        vip_speed_text = None
        dog_speed_text = None
        vip_acc_text = None
        roll_text = None
        if self.config_HA.guide.render_tension_text:
            # [수정] 개 속도를 좌측 하단 오버레이 최상단에 표시
            dog_speed_text = ax.text(
                0.02,
                0.13,
                'Dog Speed: 0.00 m/s',
                fontsize=14,
            ...
            ax.add_artist(dog_speed_text)

            # [수정] 사람 가속도를 개 속도 아래에 표시
            vip_acc_text = ax.text(
                0.02,
                0.105,
                'VIP Accel: 0.00 m/s^2',
                fontsize=14,
                transform=ax.transAxes,
                color='chocolate',
                bbox=dict(facecolor='white', alpha=0.7, edgecolor='none', pad=3.0)
            )
            ax.add_artist(vip_acc_text)

            # [수정] 사람 속도를 텐션 위쪽에 표시
            vip_speed_text = ax.text(
                0.02,
                0.08,
                'VIP Speed: 0.00 m/s',
                fontsize=14,
            ...
            ax.add_artist(vip_speed_text)

            # [수정] roll angle을 speed와 tension 사이에 표시
            roll_text = ax.text(
                0.02,
                0.055,
                'Roll: 0.00 deg',
            ...
            ax.add_artist(roll_text)

            # [수정] 하니스 텐션을 좌측 하단(axes 기준 고정 위치)에 실시간 표시
            harness_tension_text = ax.text(
                0.02,
                0.03,
                'Tension: 0.00 N',
                fontsize=14,
            ...
            ax.add_artist(harness_tension_text)
        ...
        orientations = []
        # [수정] 화살표 목록에 vip 1개를 추가(+2: dog + vip + humans)
        for i in range(self.HA_env.num_humans_in_episode + 2):
            orientation = []
        ...
        def update(frame_num):
            ...
            # [수정] 매 프레임 vip 위치 업데이트
            vip.center = vip_positions[frame_num]
            harness_line.set_data(
                [robot_positions[frame_num][0], vip_positions[frame_num][0]],
                [robot_positions[frame_num][1], vip_positions[frame_num][1]]
            )
            ...
            if dog_speed_text is not None:
                dog_speed_text.set_text('Dog Speed: {:.2f} m/s'.format(self.dog_speeds[frame_num]))
            if vip_acc_text is not None:
                vip_acc_text.set_text('VIP Accel: {:.2f} m/s^2'.format(self.vip_accels[frame_num]))
            if vip_speed_text is not None:
                vip_speed_text.set_text('VIP Speed: {:.2f} m/s'.format(self.vip_speeds[frame_num]))
            if roll_text is not None:
                roll_text.set_text('Roll: {:.2f} deg'.format(self.harness_roll_degs[frame_num]))
            if harness_tension_text is not None:
                harness_tension_text.set_text('Tension: {:.2f} N'.format(self.harness_tensions[frame_num]))
